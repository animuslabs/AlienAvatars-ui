...
import { Asset, BytesType, Name, NameType, Serializer, UInt64 } from 'anchor-link'
...
export type Edition = 'cartoon' | 'poly' | 'anime'
const emptyTables = { cartoon: [], poly: [], anime: [] }
class EmptyTables {
  cartoon:any[] = []
  poly:any[] = []
  anime:any[] = []
...
export const contractState = defineStore({
  ...
  actions: {
    ...
    async getConfig() {
      const result = (await getFullTable({ tableName: 'config' }))[0]
