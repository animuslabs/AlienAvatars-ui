        ...
        div
          ...
          .centered
            div(v-for="unpack of unpacks").q-ma-md.full-width
              ...
              .centered.full-width
                q-btn(:label="`Claim ${unpack.claimable_template_ids.length || ' '} Cards`" :disable="unpack.claimable_template_ids.length === 0" @click="claimCards(unpack.pack_asset_id)" icon="pan_tool" padding="10px" color="primary" style="width:30%; min-width:200px;" size="md").q-mt-lg.bg-secondary
        div(v-if="unpacks.length>1")
          q-separator(color="white" size="1px").q-ma-md
          .centered
            q-btn.q-mb-lg(:label="`Claim all ${totalUnclaimed} Cards at once`" @click="claimAllCards(unpacks.map(el=> el.pack_asset_id))" icon="pan_tool" padding="15px" color="primary" style="width:100%;" size="md").q-mt-lg.bg-secondary
    ...
    async 'unpacks'(val:Unpack[]) {
      for (const unpack of val) {
        for (const templateId of unpack.claimable_template_ids) {
          await this.atomic.getTemplate(templateId.toNumber())
        }
      }
    },
    'loading'(val) {
