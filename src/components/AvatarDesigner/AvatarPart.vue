...
<template lang="pug">
.centered.relative-position.items-center.q-ma-md(style="zoom:76%")
  ...
  div
    q-img.cursor-pointer(
      ...
      width="12.5em"
      height="17.5em"
      placeholder-src="/loadingCard.png"
      no-transition
      ref="img"
      fit="scale-down" @click="handlePartSelection()"
      @error="handleLoadError"
      )
  div(v-if="ownedCount>0 && !designer.filter.showDetails" :label="ownedCount" style="top:-17px;right:-17px; border-radius: 20%; height:40px;min-width:40px;").absolute.q-pa-sm.q-ma-sm.bg-info.bg-cyan-9.shadow-4
    .centered
      p.no-margin  {{ownedCount}}
    q-tooltip
      p.no-margin You Own: {{ownedCount}}
  div(v-if="ownedCount>0 && designer.filter.showDetails" :label="ownedCount" style="top:-1px;right:-8px; border-radius: 20%; height:40px;min-width:40px;").absolute.q-pa-sm.q-ma-sm.bg-info.bg-cyan-9.shadow-4
    .centered
      p.no-margin  {{ownedCount}}
  ...
  div(v-if="designer.filter.showDetails" style="width:190px;")
    q-card( style="width:220px; height:190px; right:20px; z-index: -1000; border-radius: 10px").boiddarkbg.absolute
...
export default defineComponent({
  ...
  computed: {
    ...
    imgUrl():string {
      if (!this.error) return ipfs(this.bodyPart.meta.img)
