...
.row
  .col
    q-card.relative-position(style="display: flex; flex-direction:column; min-width:220px;" ).full-width.full-height.bg-grey-10
      q-img(v-if="!statsOnly" :src="imgUrl" style="filter:opacity(.154);" noSpinner).absolute-center
      .row.justify-center
      ...
      .row.justify-center.bg-secondary
        q-btn( v-if="!statsOnly" :label="buyLabel" @click="purchase()" color="primary" :disable="disableBuy").full-width
...
export default defineComponent({
  ...
  methods: {
    ...
    async fetchMeta() {
      ...
      this.atomic.getTemplate(parseInt(this.pack.template_id.toString()))
    }
  },
  props: {
    pack: {
