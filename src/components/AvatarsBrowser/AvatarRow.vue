<template lang="pug">
.centered
  q-card.q-ma-md.q-pa-sm.full-width(style="zoom: 80%; background: radial-gradient(circle, #756F8E 40%, #3E426D 100%")
    div.full-width
      .row(style="min-height:390px;")
        .col-auto(style="width:20px;").gt-sm
        .col-auto(style="min-width:250px; height:400px;")
          ...
          .row.relative-position.cursor-pointer(@click="showFull=!showFull")
            q-img(:src="ipfs(avatar.meta.img)" style="pointer-events: none;" ref="avatarImage" )
              q-tooltip(:delay="300" style="z-index: 9999999; height:750px; width:750px;" anchor="top middle" self="top middle" :offset="[0,400]" :key="avatar.row.avatar_name.toString()" v-model="showFull").bg-transparent
            ...
            .row.justify-end.absolute-bottom(style="top:255px; right:10px; ")
              .relative-position.q-pa-sm.no-margin(style="border-radius: 100%; width:40px; height:40px; top:-90px" :class="rarityColor").shadow-7
                h5.absolute-center(style="margin-top:2px;") {{avatar.meta.rarityScore}}
                  q-tooltip(anchor="top middle" self="top middle")
                    p Rarity Score
              .column.absolute-bottom.q-pa-md.no-wrap(style="top:-60px; height:190px;")
                q-separator(color="secondary" spaced)
                .row
                  p Template Name
                .row
                  .centered
                    h5.text-capitalize {{avatar.row.avatar_name}}
                div
                  div(v-if="browser.filter.showDetails")
                    .row
                      p Designer
                    .row
                      .centered
                        h5 {{avatar.row.creator}}
                  div(v-else)
                    .row
                      p.no-margin Mint Price
                    .row
                      .centered
                        p.no-margin {{mintPrice}}
        .absolute-bottom(style="bottom:15px;" v-if="!browser.filter.showDetails")
          q-btn.full-width(:label="mintButtonText" size="lg" color="secondary" @click="mintAvatar()" :disable="disableMint")
        .col-auto(v-if="browser.filter.showDetails")
          .centered.q-gutter-sm.q-mt-sm.q-mb-sm
            .col-auto(style="width:20px;").gt-sm
            .col-auto
                q-card.q-ma-sm.q-pa-md(style="min-width:310px; max-height:360px;").full-height.boiddarkbg
                  .centered
                    p Traits | Rarity Score
                  q-separator(color="secondary" spaced)
                  .full-width.q-mb-md
                  .column.q-ml-sm
                    .row.items-center
                      ...
                      .col-auto(style="width:15px")
                        h6 {{partsMeta.top.meta.rarityScore}}
                      ...
                      .col-auto
                        h6 {{partsMeta.torso.meta.rarityScore}}
                  .q-mt-sm
                  .column.q-ml-sm
                    .row
                      .col-auto(style="width:95px;")
                        .row
                          div Head:
                        .row
                          a.text-capitalize.text-grey-3(:href="atomicHubTemplate(partsMeta.head.templateId)" target="_blank") {{avatar.meta.head}}
                      .col-auto(style="width:15px")
                        h6 {{partsMeta.head.meta.rarityScore}}
                      q-separator(color="secondary" vertical spaced="15px")
                      .col-auto(style="width:110px;")
                        .row
                          div Legs:
                    ...
                    .row
                      .col-auto(style="width:95px;")
                        .row
                          div Eyes:
                        .row
                          a.text-capitalize.text-grey-3(:href="atomicHubTemplate(partsMeta.eyes.templateId)" target="_blank") {{avatar.meta.eyes}}
                      .col-auto(style="width:15px")
                        h6 {{partsMeta.eyes.meta.rarityScore}}
                      q-separator(color="secondary" vertical spaced="15px")
                      .col-auto(style="width:110px;")
                        .row
                          div Large Wings:
                        .row
                          a.text-capitalize.text-grey-3(:href="atomicHubTemplate(partsMeta['big-wings'].templateId)" target="_blank") {{avatar.meta['big-wings']}}
                      .col-auto(style="width:10px")
                        h6 {{partsMeta['big-wings'].meta.rarityScore}}
                  .q-mt-sm
                  .column.q-ml-sm
                    .row
                      .col-auto(style="width:95px;")
                        .row
                          div Mouth:
                        .row
                          a.text-capitalize.text-grey-3(:href="atomicHubTemplate(partsMeta['mouth'].templateId)" target="_blank") {{avatar.meta['mouth']}}
                      .col-auto(style="width:15px")
                        h6 {{partsMeta['mouth'].meta.rarityScore}}
                      q-separator(color="secondary" vertical spaced="15px")
                      .col-auto(style="width:110px;")
                        .row
                          div Small Wings:
                        .row
                          a.text-capitalize.text-grey-3(:href="atomicHubTemplate(partsMeta['small-wings'].templateId)" target="_blank") {{avatar.meta['small-wings']}}
                      .col-auto(style="width:10px")
                        h6 {{partsMeta['small-wings'].meta.rarityScore}}
            .col-auto
                q-card.q-ma-sm.q-pa-md(style="min-width:200px; height:360px").boiddarkbg
                  .centered
                    p Minting
                  q-separator(color="secondary" spaced)
                  .column(style="height:290px;").no-wrap
                    .row.q-gutter-sm
                      .col-auto.q-mr-sm
                        .row
                          div Last Mint:
                    ...
                    .centered.q-mt-sm
                      .col-auto
                        q-btn(label="template" :href="atomicTemplate" type="a" icon="link" target="_blank")
                      q-separator(vertical spaced color="secondary")
                      .col-auto
                        q-btn(label="market" :href="atomicMarket" type="a" icon="link" target="_blank")
                    ...
                    div(v-if="!maxSupplyReached")
                      ...
                      .centered.full-width
                        q-btn(:label="mintButtonText" size="lg" color="secondary" @click="mintAvatar()" :disable="disableMint").full-width
...
import { atomicState, AvatarMeta, PartCardMeta, PartCardMetaClass, PartsMeta, TemplateData } from 'src/stores/AtomicStore'
...
import ms from 'ms'
import { PartsData, PartsSet } from 'src/stores/DesignerStore'
...
export default defineComponent({
  ...
  computed: {
    ...
    partsMeta():Record<Elements, {meta:PartCardMeta, templateId:number}> {
      ...
      Object.keys(parts)
        .forEach((type) => {
          data[type] = templates.find(el => el.meta.bodypart === type && el.meta.name === this.avatar.meta[type]) || { meta: new PartCardMetaClass(), templateId: 0 }
        })
      return data
    }
  },
